...
# Deploy all Lambda functions for NiryatSaathi
...
echo "========================================="
echo "Deploying NiryatSaathi Lambda Functions"
echo "========================================="
echo "Region: $REGION"
...
deploy_lambda() {
    ...
    # Note: boto3 is already included in Lambda runtime, so we can skip this for now
    if [ -f requirements.txt ]; then
        echo "Skipping pip install (boto3 is included in Lambda runtime)"
        # Uncomment below if you need to install other dependencies
...
echo ""
echo "Next steps:"
echo "1. Load HSN data: ./scripts/load-hsn-data.sh"
echo "2. Deploy API Gateway: ./scripts/deploy-api-gateway.sh"
